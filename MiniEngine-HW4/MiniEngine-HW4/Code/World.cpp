...
bool PointLight::SphereIlluminated(const Point3D& center, float radius) const
{
	...
	float r = radius + lightRange;
	return (SquaredMag(center - GetWorldPosition()) < r * r);
}
...
bool PointLight::BoxIlluminated(const Transform4D& transform, const Vector3D& size) const
{
	...
	// to divide v•s, v•t, and v•u by the magnitude of v when implementing Equation (9.16).

	Vector3D v = transform.GetTranslation() - GetWorldPosition();
	...
	float v2 = Dot(v, v);
	float m = (hx * vs + hy * vt + hz * vu) * InverseSqrt(v2) + lightRange;
	if (v2 >= m * m) return (false);

	return (Fmax(vs - hx, vt - hy, vu - hz) < lightRange);
}


FogNode::FogNode(float density, const ColorRGB& color) : Node(kNodeFog)
{
	fogDensity = density;
	fogColor = color;
}
...
void WorldManager::CalculateFogParams(UniversalParams *params)
{
	...
	params->fogParams.x = fogNode->fogDensity;
	params->fogParams.y = fc < 0 ? 1 : 0;
	params->fogParams.z = fc;
	params->fogParams.w = Fsgn(fc);
...
void WorldManager::BuildWorld(void)
{
	...
	PointLight *pointLight = new PointLight(ColorRGBA(5.0F, 5.0F, 5.0F), 120.0F);
	pointLight->nodeTransform.SetTranslation(Point3D(25.0F, 0.0F, 8.0F));
	...
	// Add a fog node to the world.

	#if 1	// Plain white fog

		FogNode *halfspaceFog = new FogNode(0.01F, ColorRGB(1.0F, 1.0F, 1.0F));

	#else	// Green death mist

		FogNode *halfspaceFog = new FogNode(0.02F, ColorRGB(0.0F, 0.125F, 0.0F));

	#endif

	halfspaceFog->nodeTransform.SetTranslation(Point3D(0.0F, 0.0F, 5.0F));
	rootNode->AppendSubnode(halfspaceFog);
}
